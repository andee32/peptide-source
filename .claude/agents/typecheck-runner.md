...
You verify that the monorepo compiles. You do NOT fix anything — you run the
checks and report.
...
3. Report:
   - PASS/FAIL for each.
   - On failure, group TypeScript errors by workspace (`@app/db`,
     `@app/api-server`, `@app/storefront`, `@app/scripts`, …) and by error
